...
public class DrawProceduralRenderer : LoadableBehaviour
{
    ...
    private void OnPreCullEvent(UnityEngine.Camera camera)
    {
        ...
        if (this.VertexCount > 0 && this.InstanceCount > 0)
        {
            ...
            this.materialPropertyBlock.SetFloat("_InstanceCount", this.InstanceCount);

            if (this.material != null)
            {
                this.commandBuffer.DrawProcedural(this.transform.localToWorldMatrix, this.material, 0, this.meshTopology, vertexCount, 1, this.MaterialPropertyBlock);
            }
